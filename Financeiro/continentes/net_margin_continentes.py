"""
Net Margin por Continente/Região — evolução anual.
Fórmula: (Net Profit / Revenue) * 100
Gera 2 ficheiros separados:
  net_margin_continentes.png       → linhas por região
  net_margin_continentes_stats.png → tabela de estatísticas acumuladas
"""
...
region_data["Net_Margin"] = (region_data["Net_Profit"] / region_data["Revenue"]) * 100

pivot = region_data.pivot(index="Year", columns="Region", values="Net_Margin").fillna(0)
years = pivot.index.tolist()
regions = pivot.columns.tolist()
n_years = len(years)

COLORS = {"Asia": "#1f77b4", "Europe": "#ff7f0e", "North America": "#2ca02c"}
...
fig.suptitle("Net Margin por Região — Evolução Anual (%)", fontsize=14, fontweight="bold")
...
ax.set_ylabel("Net Margin (%)", fontsize=11)
ax.legend(title="Região", loc="upper left")
ax.grid(linestyle="--", alpha=0.4)
ax.set_xlim(years[0]-0.2, years[-1]+0.8)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "net_margin_continentes.png"), dpi=150)
plt.close()

# ────────────────────────────────────────────────────────────────────────────────
# PNG 2 — Tabela de estatísticas
# ────────────────────────────────────────────────────────────────────────────────
row_labels = ["Média de Margem Líq.", "CAGR da Margem Líq.", "Cresc. Médio YoY", "Pico (ano)", "Mínimo (ano)"]
cell_data = [
...
ax.axis("off")
fig.suptitle("Estatísticas — Net Margin por Região", fontsize=13, fontweight="bold", y=0.97)
...
plt.savefig(os.path.join(OUTPUT_DIR, "net_margin_continentes_stats.png"), dpi=150, bbox_inches="tight")
plt.close()
