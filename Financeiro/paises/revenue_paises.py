"""
Revenue por País — 3 regiões × 3 ficheiros cada:
...
def plot_region(region_key: str, config: dict):
    ...
    data  = country_revenue[country_revenue["Region"] == region_key]
    ...
    MIN_FRAC      = 0.04   # omite anotação apenas se a banda for < 4% do total
    FRAC_NORMAL   = 0.13   # acima disto → fonte normal (8.5)
    FONT_SMALL    = 6.0    # fonte para bandas entre 4% e 13%
    FONT_NORMAL   = 8.5    # fonte para bandas >= 13%
    PCT_NORMAL    = 10     # acima disto → fonte normal (9) no gráfico %
    PCT_SMALL_MIN = 4      # abaixo disto → omite no gráfico %
    FONT_PCT_SMALL  = 6.0
    FONT_PCT_NORMAL = 9.0
    ...
    # Total do continente — caixa de texto acima do topo
    ...
    # Valor dentro de cada banda — fonte menor para bandas estreitas
    ...
    plt.tight_layout()
    p = os.path.join(OUTPUT_DIR, f"revenue_paises_{slug}_abs.png")
    ...
    plt.tight_layout()
    p = os.path.join(OUTPUT_DIR, f"revenue_paises_{slug}_pct.png")
    plt.savefig(p, dpi=150, bbox_inches="tight")
    print(f"Guardado: {p}")
    plt.close()
    ...
    fig_h   = 1.2 + n_rows * 0.55 + 0.6   # altura proporcional ao nº de linhas
    fig, ax = plt.subplots(figsize=(max(8, 3.5 * n_cols + 3), fig_h))
    ax.axis("off")
    fig.suptitle(
        f"Estatísticas Acumuladas por País — {label}",
    ...
    # Cabeçalho de colunas com cor da região
    ...
    plt.tight_layout()
    p = os.path.join(OUTPUT_DIR, f"revenue_paises_{slug}_stats.png")
    plt.savefig(p, dpi=150, bbox_inches="tight")
    print(f"Guardado: {p}")
    plt.close()
